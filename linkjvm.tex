...
While libkovan mostly avoids the real challanges in coding C, when implementing complex algorithms there is no way around pointers and memory management.
...
While the Java\cite{Java} environment JamVM and GNU Classpath\cite{GNU} works pretty well the robot library is not that good.
...
The most important of them is that there is no possibility to write a thread save library using this approach, because libkovan\cite{libkovan} itself is not thread-save.
